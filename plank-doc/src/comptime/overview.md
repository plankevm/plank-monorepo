...
Use `@compile_error(message)` to stop compilation with a custom diagnostic when an invalid comptime path is evaluated. The message is a `cbytes` value, so string literals and compile-time bytes constants can be used.

```plank
const only_u256 = fn (comptime T: type) void {
    if T != u256 {
        @compile_error("only_u256 expects u256");
    }
};
```

This is useful for type-driven libraries that need to reject unsupported types clearly instead of relying on incidental type errors.
