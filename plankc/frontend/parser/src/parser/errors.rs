...
impl<'a> Parser<'a> {
    ...
    pub(crate) fn emit_path_in_import_group(&mut self, path_start: TokenIdx) {
        ...
        let end = self.last_src_span.end;
        let diag = Diagnostic::error("path in import group")
            .primary(
                self.source_id,
                Span::new(start, end),
    ...
    pub(crate) fn emit_glob_in_import_group(&mut self) {
        let diag = Diagnostic::error("glob import inside import group")
            .primary(
                self.source_id,
                self.last_src_span,
    ...
    pub(super) fn emit_unnecessary_braces(&mut self, brace_start: TokenIdx) {
        ...
        let diag = Diagnostic::warning("unnecessary braces in import")
