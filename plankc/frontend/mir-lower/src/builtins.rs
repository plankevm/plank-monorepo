...
use plank_session::{Builtin, RuntimeBuiltin};
use sir_data::{
    self as sir,
...
pub(crate) fn add_as_op(
    ...
    let kind = match builtin.inner() {
        ...
        | Builtin::GetField
        | Builtin::SetField => {
            unreachable!("comptime/polymorphic builtin is not a runtime builtin: {builtin}")
        }
    };
    let op_extra_data = match builtin.inner() {
