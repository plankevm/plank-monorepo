...
contract PrecompileTestTest is BaseTest {
    ...
    function test_ecrecover() public {
        ...
        assertEq(
            bytes32(out),
            bytes32(uint256(uint160(ecrecover(hash, v, r, s))))
        );
    }
    ...
    function test_ecrecover_invalid() public {
        ...
        assertEq(
            bytes32(out),
            bytes32(uint256(uint160(ecrecover(0, 0, 0, 0))))
        );
    }
    ...
    function test_sha256() public {
        ...
        (bool succ, bytes memory out) = plankImpl.call(
            abi.encodeWithSignature("sha256Ext(bytes)", data)
        );
        assertEq(succ, true);
    ...
    function test_ripemd160() public {
        bytes memory data = abi.encode(0xdeadbeef);
        (bool succ, bytes memory out) = plankImpl.call(
            abi.encodeWithSignature("ripemd160Ext(bytes)", data)
        );
        assertEq(succ, true);
