...
impl<'a, 'hir> BodyLowerer<'a, 'hir> {
    ...
    fn emit_set(&mut self, ty: TypeId, value: mir::Expr) -> mir::LocalId {
        ...
        self.instructions_buf.push(mir::Instruction::Set { target, expr: value });
        target
    }
            ...
            hir::Expr::BuiltinCall { builtin, args: call_args_id } => {
                ...
                let return_type = signatures
                    ...
                    .find_map(|(inputs, output)| {
                        if inputs.len() != actual_types.len() {
                            return None;
                        }
                        inputs
                            .iter()
                            .zip(actual_types)
                            .all(|(expected, actual)| actual.is_assignable_to(*expected))
                            .then_some(*output)
                    })
                    .unwrap_or_else(|| {
    ...
    fn walk_sub_block(&mut self, block_id: hir::BlockId) -> mir::BlockId {
        ...
        self.walk_instructions(block_id);
        let instructions = self.instructions_buf.drain(instructions_start..);
        self.eval.mir_blocks.push_iter(instructions)
    }

    fn walk_instructions(&mut self, block_id: hir::BlockId) -> BlockExit {
        for &instr in &self.eval.hir.blocks[block_id] {
            if self.walk_instruction(instr) == BlockExit::Never {
                return BlockExit::Never;
            }
        }
        BlockExit::Return
    }

    fn walk_instruction(&mut self, instr: hir::Instruction) -> BlockExit {
        match instr {
            ...
            hir::Instruction::Eval(expr) => {
                let ty = match self.translate_expr(expr) {
                    ExprResult::ComptimeOnly(_) => return BlockExit::Return,
                    ExprResult::Runtime { ty, .. } => ty,
                };
                if ty == TypeId::NEVER { BlockExit::Never } else { BlockExit::Return }
            }
            ...
            hir::Instruction::If { condition, then_block, else_block, result } => {
                ...
                match cond_comptime {
                    Some(ValueId::TRUE) => self.walk_instructions(then_block),
                    Some(ValueId::FALSE) => self.walk_instructions(else_block),
                    ...
                    None => {
                        ...
                        if cond_ty.is_none_or(|ty| !ty.is_assignable_to(TypeId::BOOL)) {
            ...
            hir::Instruction::Assign { target, value } => {
                ...
                if !rhs_ty.is_assignable_to(target_ty) {
                    todo!(
...
fn lower_fn_body(eval: &mut Evaluator<'_>, closure_value_id: ValueId) -> mir::FnId {
    ...
    if lowerer.walk_instructions(fn_def.body) == BlockExit::Never {
        lowerer.return_type = None;
    }
    if let Some(actual) = lowerer.return_type
        && !actual.is_assignable_to(return_type)
    {
...
pub(crate) fn lower_entry_point_as_fn(
    ...
    let mut lowerer = BodyLowerer::new(eval);
    if lowerer.walk_instructions(hir_block) != BlockExit::Never {
        todo!(
